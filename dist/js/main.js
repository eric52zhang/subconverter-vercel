(() => {
	...
	function t(e) {
		const l = e.backend;
		let i = e.url;
		i = encodeURIComponent(i.replace(/(\n|\r|\n\r)/g, "|"));
		let t = `${l}&url=${i}&target=${e.target}`;
		var a;
		e.config && (t += `&config=${encodeURIComponent(e.config)}`), e.include && (t += `&include=${encodeURIComponent(e.include)}`), e.exclude && (t += `&wxclude=${encodeURIComponent(e.exclude)}`), e.name && (t += `&filename=${encodeURIComponent(e.name)}`), t += `&emoji=${e.emoji||"false"}&append_type=${e.append_type||"false"}&append_info=${e.append_info||"false"}&scv=${e.scv||"false"}&udp=${e.udp||"false"}&list=${e.list||"false"}&sort=${e.sort||"false"}&fdn=${e.fdn||"false"}&insert=${e.insert||"false"}`, n = t, $("#result").val(n), a = n, navigator.clipboard.writeText(a).then((() => {
			layui.layer.msg("复制成功~", {
				icon: 1
			})
		}), (() => {
			layui.layer.msg("复制失败, 请手动选择复制", {
				icon: 2
			})
		}))
	}
	layui.use(["form"], (() => {
		...
		let o = "";
		e.forEach((e => {
			o += `<option value="${e.value}">${e.label}</option>`
		})), $("#targetSelecter").append(o), o = "", i.forEach((e => {
			let l = "";
			e.options.forEach((e => {
				l += `<option value="${e.value}">${e.label}</option>`
			})), o += `<optgroup label="${e.label}">${l}</optgroup>`
		})), $("#configSelecter").append(o), o = "", l.forEach((e => {
			o += `<option value="${e.value}">${e.label}</option>`
		})), $("#backendSelecter").append(o), a.render("select", "optionsForm"), a.on("submit(generate)", (e => {
			t(e.field)
		})), $("#importToClash").on("click", (() => {
			if (!n) return layui.layer.msg("未生成新的订阅链接", {
				icon: 2
			});
			const e = `clash://install-config?url=${encodeURIComponent(n)}`;
			window.open(e)
		}))
	}))
})();
